...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
       ...
       System.out.println(consolReader1.isInterrupted()+"-"+consolReader1.isAlive());
        System.in.close();// Это вообщето в конце этого цикла
        reader.close();//  Это вообщето в конце этого цикла
        ...
        System.out.println(consolReader1.isInterrupted()+"-"+consolReader1.isAlive());
        System.out.println(consolReader2.isInterrupted()+"-"+consolReader2.isAlive());
        System.out.println(consolReader3.isInterrupted()+"-"+consolReader3.isAlive());
    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            ...
            while (!/*current.*/isInterrupted()){
                try
                {
                    if (reader.ready()) // Проверяет готов ли reader к чтению (true- готов), так как Sistem.in и reader закрыли в основном методе - close, выдает false НЕВСЕГДА!!!
                    {
                      //  System.out.println(getName()+ " "+"Попал"); // Демонстрирует работу потоков
                    result.add(reader.readLine());
                    ++countReadStrings;
                       // System.out.println(getName()+ "Сделал - "+ Integer.toString(countReadStrings)); // Демонстрирует работу потоков
                    }
                }
                catch (IOException e)
                {
                   /* e.printStackTrace();
                    System.out.println(getName()+ "Прерван "+ Integer.toString(countReadStrings)); // Демонстрирует работу потоков
*/
